use calc::{
    Color, ColorState, Configuration, simulate_n_rounds_full, simulate_rounds, simulate_rounds_full,
};
...
fn test_simulate_rounds_debug() {
    ...
    let res = simulate_rounds_full(init_conf);
    println!("{res:?}");
    let all_game_states_count: u64 = res[0].iter().sum();
...
#[test]
fn test_simulate_rounds_n_debug() {
    let init_conf = Configuration::builder()
        .with_map(vec![
            (8, Color::Blue),
            (8, Color::Green),
